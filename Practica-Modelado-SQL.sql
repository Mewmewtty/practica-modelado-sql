...
create table flota.coche (
	...
	kms_totales decimal(10,2) not null,
...
create table flota.revision (
	...
	importe_revision decimal(10,2) not null,
	kms_revision decimal(10,2) not null,
...
alter table flota.coche add constraint fk_modelo foreign key (id_modelo) references flota.modelo(id_modelo);
alter table flota.coche add constraint fk_color foreign key (id_color) references flota.color(id_color);
alter table flota.modelo add constraint fk_marca foreign key (id_marca) references flota.marca(id_marca);
alter table flota.marca add constraint fk_grupo foreign key (id_grupo) references flota.grupo(id_grupo);
alter table flota.poliza add constraint fk_aseguradora foreign key (id_aseguradora) references flota.aseguradora(id_aseguradora);
alter table flota.poliza add constraint fk_matricula foreign key (matricula) references flota.coche(matricula);
alter table flota.revision add constraint fk_matricula foreign key (matricula) references flota.coche(matricula);
alter table flota.revision add constraint fk_moneda foreign key (id_moneda) references flota.moneda(id_moneda);
...
insert into flota.marca(nombre_marca, id_grupo)
select c.marca, g.id_grupo
from flota.coches c
inner join flota.grupo g on c.grupo = g.nombre_grupo
group by g.id_grupo, c.marca;

insert into flota.modelo(nombre_modelo, id_marca)
select c.modelo, m.id_marca
from flota.coches c
inner join flota.marca m on c.marca = m.nombre_marca
group by m.id_marca, c.modelo;

insert into flota.aseguradora(nombre_aseguradora)
select aseguradora from flota.coches
group by aseguradora;

insert into flota.coche(matricula, fecha_compra, kms_totales, id_color, id_modelo)
select c.matricula, c.fecha_compra, c.kms_totales, co.id_color, m.id_modelo
from flota.coches c
inner join flota.color co on c.color = co.nombre_color
inner join flota.modelo m on c.modelo = m.nombre_modelo 
group by c.matricula, c.fecha_compra, c.kms_totales, co.id_color, m.id_modelo
order by c.matricula;

insert into flota.poliza(n_poliza, fecha_alta_seguro, id_aseguradora, matricula)
select c.n_poliza, c.fecha_alta_seguro, a.id_aseguradora, ch.matricula
from flota.coches c
inner join flota.aseguradora a on c.aseguradora = a.nombre_aseguradora 
inner join flota.coche ch on c.matricula = ch.matricula
group by c.n_poliza, c.fecha_alta_seguro, a.id_aseguradora, ch.matricula 
order by c.n_poliza;

insert into flota.revision(importe_revision, kms_revision, fecha_revision, matricula, id_moneda)
select c.importe_revision, c.kms_revision, c.fecha_revision, c.matricula, md.id_moneda
from flota.coches c
inner join flota.coches ch on c.matricula = ch.matricula 
inner join flota.moneda md on c.moneda = md.nombre_moneda 
group by c.importe_revision, c.kms_revision, c.fecha_revision, c.matricula, md.id_moneda 
order by c.matricula;

drop table flota.coches;


-- Consulta --

select md.nombre_modelo, mc.nombre_marca, g.nombre_grupo, c.fecha_compra, c.matricula, co.nombre_color, c.kms_totales, a.nombre_aseguradora, p.n_poliza
from flota.coche c
inner join flota.modelo md on c.id_modelo = md.id_modelo 
inner join flota.marca mc on md.id_marca = mc.id_marca
inner join flota.grupo g on mc.id_grupo = g.id_grupo 
inner join flota.color co on c.id_color = co.id_color 
inner join flota.poliza p on c.matricula = p.matricula
inner join flota.aseguradora a on p.id_aseguradora = a.id_aseguradora 
group by md.nombre_modelo, mc.nombre_marca, g.nombre_grupo, c.fecha_compra, c.matricula, co.nombre_color, c.kms_totales, a.nombre_aseguradora, p.n_poliza
order by c.matricula 
